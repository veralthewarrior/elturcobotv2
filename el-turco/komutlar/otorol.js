...
exports.run = async(client, message, args) => {
    var bymayfe = args[0];
  if(!bymayfe) return message.channel.send("** :x: Bir seçenek belirtmen gerek sahip!** (**ayarla** - **kapat**)");
 ...
 let rol = message.mentions.roles.first() || message.guild.roles.get(args.join(' '))
  let newRole;
  let tworole;
  if (!rol) return message.channel.send(' :x: Otorol ayarlamak için bir rol etiketle sahip! `-otorol ayarla @Üye #kanal`')
  ...
  if (!otorolkanal) return message.channel.send(' :x: Otorol ayarlamak için bir rol etiketle sahip! `-otorol ayarla @Üye #kanal`')
  ...
  if (!message.guild.roles.get(newRole)) return message.channel.send(" :x: Etiketlediğin rolü bulamadım sahip. Bu rolün etiketlenebilirliğinden veya aktifliğinden emin olman gerek.")
